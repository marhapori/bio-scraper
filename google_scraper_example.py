...
def search_termeszetes(query):
    ...
    """
    url = f"https://www.termeszetes.com/en/?s={urllib.parse.quote_plus(query)}"
    user_agent = (
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
        "AppleWebKit/537.36 (KHTML, like Gecko) "
        "Chrome/90.0.4430.93 Safari/537.36"
...
def scrape_product_info(url):
    ...
    try:
        ...
        if meta and meta.get('content'):
            ...
            desc_div = soup.find(lambda tag: (
                tag.name in ['div','p'] and tag.get('class') and any(
                    'description' in c.lower() for c in tag.get('class')
                )
            ))
            if desc_div:
...
def fetch_product_data(ean, name):
    ...
    if link:
        ...
        data = { 'EAN':ean, 'title':title, 'link':link, **scraped }
        # fallback if missing
        if not data['description'] or not data['ingredients']:
            fb = fetch_bionaturorganikus(name)
            if fb:
                for key in ['title','link','ingredients','effects','packaging','description']:
                    if not data.get(key) and fb.get(key): data[key] = fb[key]
        return data
    # 2) Fallback
    fb = fetch_bionaturorganikus(name)
    if fb:
...
def main():
    ...
    raw_df['DescriptionHTML'] = raw_df.apply(
        lambda prod: tpl.render(
            product_name=prod['title'], description=prod['description'],
            ingredients=prod['ingredients'], effects=prod['effects'],
            packaging=prod['packaging']), axis=1)
